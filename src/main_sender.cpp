...
void print_usage(const char* program_name) {
    std::cout << "Usage: " << program_name << " [OPTIONS]\n"
              ...
              << "\n"
              << "Examples:\n"
              << "  " << program_name << " -i data.bin -r 192.168.1.100 -p 8080 -u -s 1024\n"
              << "  " << program_name << " -i data.bin -u -l -e -v\n"
              << "  " << program_name << " -i data.bin -u -c -d ram\n"
              << std::endl;
}
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        } else if (arg == "-l" || arg == "--link-monitor") {
    ...
    if (input_file.empty() && !config.continuous_mode) {
        std::cerr << "Error: Input file is required (unless in continuous mode)" << std::endl;
        print_usage(argv[0]);
        return 1;
    }
    ...
    std::cout << "=== High-Throughput UDP Stream Sender ===" << std::endl;
    std::cout << "Configuration:" << std::endl;
    std::cout << "  Remote Address: " << config.remote_addr << ":" << config.port << std::endl;
    ...
    if (!input_file.empty()) {
        ...
        std::cout << "  File Size: " << get_file_size(input_file) << " bytes" << std::endl;
    }
    std::cout << std::endl;
    ...
    ErrorCode result = sender.connect();
    if (result != ErrorCode::SUCCESS) {
        std::cerr << "Failed to connect: " << get_error_string(result) << std::endl;
        return 1;
    }
    
    // Start transfer in background
    std::thread transfer_thread([&sender]() {
        ErrorCode result = sender.send_file();
        if (result != ErrorCode::SUCCESS) {
            std::cerr << "Transfer failed: " << get_error_string(result) << std::endl;
            running = false;
        }
    });
    
    // Monitor transfer progress
    std::thread monitor_thread([&sender]() {
        while (running) {
            std::this_thread::sleep_for(std::chrono::seconds(5));
            
            if (!running) break;
            
            StreamStats stats = sender.get_stats();
            double throughput_gbps = convert_to_gbps(stats.throughput);
            
            std::cout << "Progress: " << stats.bytes_sent << " bytes sent, "
                      << std::fixed << std::setprecision(2) << throughput_gbps << " Gbps, "
                      << stats.packets_sent << " packets sent, "
                      << stats.packets_retransmitted << " retransmitted, "
    ...
    StreamStats final_stats = sender.get_stats();
    ...
    std::cout << "Packets Retransmitted: " << final_stats.packets_retransmitted << std::endl;
    ...
    std::cout << "Resync Count: " << final_stats.resync_count << std::endl;
    
    if (final_stats.fec_packets_sent > 0) {
        std::cout << "FEC Packets Sent: " << final_stats.fec_packets_sent << std::endl;
    }
    
    std::cout << "Sender shutdown complete" << std::endl;
    return 0;
}