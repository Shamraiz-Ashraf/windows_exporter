...
func main() {
	...
	if config.OutputFile == "" && !config.ContinuousMode {
		fmt.Fprintf(os.Stderr, "Error: Output file is required (unless in continuous mode)\n")
		flag.Usage()
		os.Exit(1)
	}
	...
	if config.UseUDP {
		receiver = stream.NewUDPReceiver(config)
		logger.Info("Using UDP receiver")
	} else {
		receiver = stream.NewReceiver(config)
		logger.Info("Using TCP receiver")
	}
	...
	go func() {
		...
		for {
			...
			case <-ticker.C:
				...
				logger.Infof("Progress: %d bytes received, %.2f Gbps, %d packets received, %d lost, Link: %v",
					stats.BytesReceived, throughputGbps, stats.PacketsReceived, stats.PacketsLost, stats.LastLinkStatus)
			case <-receiveDone:
				return
			}
		}
